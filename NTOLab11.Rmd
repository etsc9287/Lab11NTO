...
ggplot() +
  geom_point(data = poop2, mapping = aes(x = difference, y = fct_reorder(name, difference))) +
...
ggplot(data = teamx, mapping = aes(x = year, y= diff,color = name))+ 
  geom_line(size = 1)+ggtitle("Popularity of Names") +xlab("Year")+ ylab("Popularity Difference") 
```

* The graph above shows the popularity difference fromt the year of birth. For example: If born in 1999, the populariry differnce for 2005 is (popularity in 2005-popularity in 1999). The graph shows the popularity trends for all names in respect to thier birth year.
* Ethan's name seems to have gained the more popularity in the years after his birth. However, it seems to be returning to its orginal popularity.
* Anderson's name shows a slight but constant increase in popularity.
* Because Arie's name is the least popular, it makes sense that there is not a disctict trend. However, the data tibble shows a very slight increase in her name popularity.
* Both Ryan and David show a steady and steep decrease in popularity compared to thier birth year.
